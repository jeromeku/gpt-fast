...
def get_shape_and_dtype(i):
    if isinstance(i, torch.Tensor):
        return i.shape, i.dtype
    return i
...
class CustomFlopCounterMode(TorchDispatchMode):
    """
    ...
    def __init__(
        ...
        self.memory_counts = defaultdict(lambda: defaultdict(int))
    ...
    def get_data_movement_counts(self) -> Dict[str, Dict[Any, List[int]]]:
        """Return the data movement counts as a dictionary of dictionaries.
        ...
        Returns:
            Dict[str, Dict[Any, List[int]]]: The data movement counts as a dictionary.
        """
        return {k: dict(v) for k, v in self.data_movement_counts.items()}
    
    def get_memory_counts(self) -> Dict[str, Dict[Any, List[int]]]:
        """Return the data movement counts as a dictionary of dictionaries.
    ...
    def __torch_dispatch__(self, func, types, args=(), kwargs=None):
        ...
        # self._count_data_movement(func._overloadpacket, out, args, kwargs)
        return self._count_flops(func._overloadpacket, out, args, kwargs)

    def _get_memory_sizes(self, args):
    ...
    def _count_data_movement(self, func_packet, out, args, kwargs):
        arg_sizes = self._get_memory_sizes(args)
        kwargs_sizes = self._get_memory_sizes(kwargs.values())
        out_sizes = self._get_memory_sizes(out)
        # print(f"arg_sizes = {arg_sizes}, kwargs_sizes = {kwargs_sizes}, out_sizes = {out_sizes}")
        arg_size, kwargs_size, out_size = sum(arg_sizes), sum(kwargs_sizes), sum(out_sizes)
        # print(f"arg_size = {arg_size}, kwargs_size = {kwargs_size}, out_size = {out_size}")
        # for par in set(self.mod_tracker.parents):
        #     if self.debug:
        #         print(f"Counting data movement for {par}, {func_packet}: {arg_size}, {kwargs_size}, {out_size}")
        #     self.data_movement_counts[par][func_packet] += arg_size + kwargs_size + out_size        
    ...
    def _count_flops(self, func_packet, out, args, kwargs):
        if func_packet in self.flop_registry:
            ...
            total_size = arg_size + kwarg_size if self.weights_only else arg_size + kwarg_size + out_size
            ...
            for par in set(self.mod_tracker.parents):
                ...
                self.memory_counts[par][func_packet] += total_size
        return out
...
if __name__ == "__main__":
    ...
    # total_size = a.numel() * a.element_size() + b.numel() * b.element_size() + m * n * dtype.itemsize
    ...
    # _ = model(input_ids)
    # from torch.utils.flop_counter import FlopCounterMode
    with CustomFlopCounterMode(display=False, depth=10) as flop_counter:
        _ = model(input_ids)
    
    print(f"Total flops: {flop_counter.get_total_flops()}")
    print(f"Total memory bytes: {flop_counter.get_total_memory()}")
    model_params = model.num_parameters()
    model_dtype_size = next(model.parameters()).element_size()
    print(f"Total model params, memory: {model_params} {model_params * model_dtype_size}")
    print(f"Total size debug: {flop_counter._total_size}")
    #     # c = torch.matmul(a, b)
    # flop_counts = flop_counter.get_flop_counts()
    # memory_counts = flop_counter.get_memory_counts()
    # for k, v in flop_counts.items():
    #     print(f"{k}: {v}")
    # for k, v in memory_counts.items():
    #     print(f"{k}: {v}")
    # print(flop_counter.get_flop_counts()['Global']) 
    # print(flop_counter.get_memory_counts()['Global'])
    # lm_head_inputs = 128 * model.config.hidden_size * model.lm_head.weight.element_size()
    # lm_head_weights = model.lm_head.weight.numel() * model.lm_head.weight.element_size() 
    # lm_head_outputs = 128 * model.config.vocab_size * model.lm_head.weight.element_size()
    # print(f"lm_head_size = {lm_head_inputs} + {lm_head_weights} + {lm_head_outputs} = {lm_head_inputs + lm_head_weights + lm_head_outputs}")
